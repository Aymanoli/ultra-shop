...
const loadProducts = () => {
  ...
  const universalData = loadProducts();
  
  // Show product Details
  const showDetails = id => {
    const productDetailsContainer = document.getElementById('product-details');
    productDetailsContainer.textContent = '';
    const currentProduct = universalData.find(product => product.id === id)
    console.log(currentProduct);
    productDetailsContainer.innerHTML = `
      <div class="row mb-4 border py-2">
        <div class="col-3">
          <div class="border text-center p-2" style="height: 370px;">
            <img class="h-100 img-fluid" src="${currentProduct.image}">
          </div>
        </div>
        <div class="col-9">
          <div style="height: 370px;">
            <h2>${currentProduct.title}</h2>
            <small class="text-secondary">Category: <span>${currentProduct.category}</span></small>
            <p>${currentProduct.description}</p>
            <p class="text-primary">Rating: <span>${currentProduct.rating.rate}</span> out of 5.0 (${currentProduct.rating.count} reviews)</p>
            <p class="fs-4"><strong>$<span>${currentProduct.price}</span></strong></p>
            <button onclick="addToCart(${currentProduct.id},${currentProduct.price})" id="addToCart-btn"
  ...
  const showProducts = (products) => {
    ...
    for (const product of allProducts) {
      const image = product.image; // product image fixed
      ...
      <div class="myCard h-100">
        ...
        <div class="card-body">
          ...
          <p class="text-primary">Rating: ${product.rating.rate}/5.0 (${product.rating.count})</p>
        </div>
        <div class="card-footer">
            ...
            <a href="#product-details"><button onclick="showDetails(${product.id})" id="details-btn" class="btn btn-info">Details</button></a>
        </div>
      </div>
      `;
  ...
  const precisionChecking = value => {
    return parseFloat(value) === parseInt(value) ? parseInt(value) : value.toFixed(2);
  }
  ...
  const addToCart = (id, price) => {
    ...
    updateTaxAndCharge();
    document.getElementById("total-Products").innerText = (count > 9) ? count : ('0' + count);
  };
  ...
  const updatePrice = (id, value) => {
    ...
    document.getElementById(id).innerText = precisionChecking(total);
  };
  
  // set innerText function
  const setInnerText = (id, value) => {
    document.getElementById(id).innerText = precisionChecking(value);
  };
  ...
  const updateTotal = () => {
    ...
    document.getElementById("total").innerText = precisionChecking(grandTotal); // total price precision fixed
  };
  
  // initial call total price function
  updateTotal();
  
  // Display product list from API
  showProducts(universalData);